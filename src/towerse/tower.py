...
class TowerWindDrag(Component):
    ...
    cd_usr = Array(iotype='in', desc='User input drag coefficient to override Reynolds number based one')
...
class AeroHydroLoads(Component):
    ...
    windLoads = VarTree(AeroLoads(), iotype='in', desc='wind loads in inertial coordinate system')
    waveLoads = VarTree(AeroLoads(), iotype='in', desc='wave loads in inertial coordinate system')
    ...
    q = Array(iotype='out', units='N/m**2', desc='dynamic pressure')
    ...
    def execute(self):
        ...
        self.Px = np.interp(self.z, wind.z, windLoads.x) + np.interp(self.z, wave.z, waveLoads.x)
        self.Py = np.interp(self.z, wind.z, windLoads.y) + np.interp(self.z, wave.z, waveLoads.y)
        self.Pz = np.interp(self.z, wind.z, windLoads.z) + np.interp(self.z, wave.z, waveLoads.z)
        self.q = np.interp(self.z, wind.z, wind.q) + np.interp(self.z, wave.z, wave.q)
...
class TowerFrame3DD(Component):
    ...
    weldability = Array(iotype='out')
    manufactuability = Array(iotype='out')
    ...
    def execute(self):
        ...
        Tzz = np.concatenate([[-Tzz[0]], Tzz[1::2]])
        ...
        hoop_stress = hoopStressEurocode(self.z, self.d, self.t, self.L_reinforced, self.q)
        ...
        self.damage = fatigue(M_DEL, N_DEL, self.d, self.t, self.m_SN, self.DC, self.gamma_fatigue,
            stress_factor=1.0, weld_factor=True)
...
class TowerSE(Assembly):
    ...
    def configure(self):
        ...
        self.connect('distLoads1.q', 'tower1.q')
        ...
        self.connect('distLoads2.q', 'tower2.q')
